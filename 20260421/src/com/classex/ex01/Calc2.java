...
/*
 * 문] 여러개의 클래스를 활용하여 프로그램을 구현하시오.
 * 		더하기,빼기,곱하기,나누기를  수행하는 각 클래스의 이름을 만드시오.
 * 		class → Add,sub,Mul,Div
 * 
 * 		각 클래스마다 구성되는 필드와 메소드는 동일하다.
 * 		int 타입(자료형)의 a,b 필드 → 피 연산자(첫번째 , 두번째 )
 * 		void setValue(int a,int b) 피연산의 값을 객체 저장한다.
 ...
 * 		메소드(setValue,Calculate)를 호출하여 결과를 출력하는프로그램을 구한다.
 * 
 * 		출력결과
 * 		두 정수와 연산자 입력 : 5 7 
 * 		35
...
public class Calc2 {
	...
	public static void main(String[] args) {
		...
		while(true)
		{
			...
			//Test
			//System.out.println(number01 + "," + number02 + "," + operate);
			switch (operate) {
			case "+":
				Add ad = new Add(number02, number01);
				System.out.println(ad.Calculate());
				break;
			case "-":
				Sub su = new Sub(number02, number01);
				System.out.println(su.Calculate());
				break;
			case "*":
				Mul mu = new Mul(number02, number01);
				System.out.println(mu.Calculate());
				break;
			case "/":
				Div di = new Div(number02, number01);
				System.out.println(di.Calculate());
				break;
			default:
				System.out.println("연산자 잘못입력했습니다.");
				break;
			}
		}
	}
